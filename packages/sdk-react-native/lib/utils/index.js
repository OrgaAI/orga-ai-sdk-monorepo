...
const getMediaConstraints = (config = {}) => {
    ...
    const video = {
        facingMode,
        ...exports.MIN_MAX_IDEAL_VIDEO_CONSTRAINTS[videoQuality],
    };
    return {
        audio: false,
        video,
...
const connectToRealtime = async ({ ephemeralToken, peerConnection, gathered, }) => {
    ...
    const { voice, model, temperature, enableTranscriptions, instructions, modalities, } = config;
    ...
    exports.logger.debug(`[OrgaAI] Enable Transcriptions: ${enableTranscriptions}`);
