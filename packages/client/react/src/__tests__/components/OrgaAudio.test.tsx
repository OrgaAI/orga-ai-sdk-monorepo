...
describe('OrgaAudio', () => {
  ...
  describe('Logging', () => {
    it('should log debug message when stream changes', () => {
      const { logger } = require('@orga-ai/core');
      
      render(<OrgaAudio stream={mockStream} data-testid="audio-element" />);
      
      expect(logger.debug).toHaveBeenCalledWith('OrgaAudio stream:', mockStream);
    });
    ...
    it('should log debug message when stream is null', () => {
      ...
      expect(logger.debug).toHaveBeenCalledWith('OrgaAudio stream:', null);
    });
  });
